...
public class KMeans {
    ...
    public KMeans(Cluster[] clusters, Punto[] puntos, int iteraciones, int p) {
    ...
    public void imprimirPuntos() {
        ...
        for (int i = 0; i < clusters.length; i++) {
            ...
            for (int j = 0; j < clusters[i].getPuntos().size(); j++) {
                ...
                // solo se agregan o no saltos de linea si es el ultimo elemento
    ...
    public void imprimirPalabras() {
        ...
        for (int i = 0; i < clusters.length; i++) {
            ...
            for (int j = 0; j < clusters[i].getPuntos().size(); j++) {
                imprimible += clusters[i].getPuntos().get(j).getPalabra();
                // solo se agregan o no saltos de linea si es el ultimo elemento
                imprimible += (i == clusters.length - 1 && j == clusters[i].getPuntos().size() - 1 ? "" : "\n");
            }
            imprimible += (i < clusters.length - 1 ? "\n" : "");
